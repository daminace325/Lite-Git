import argparse #to parse command-line arguments since git is a CLI
import configparser #to read and write these files since git is configuration file
from datetime import datetime #for date/time manipulation
import grp, pwd #to display users/group database
from fnmatch import fnmatch #to filename similar to .gitignore 
import hashlib #since git uses SHA-1
from math import ceil
import os #for filesystem abstraction
import re #regular expression
import sys #to access command-line argument
import zlib #git compresses everything to zlib.


argparser = argparse.ArgumentParser(description="TA simple content tracker")
...
def tag_create(repo, name, ref, create_tag_object=False):
    ...
    if create_tag_object:
        ...
        tag.kvlm[b'tagger'] = b'lite <lite@example.com>'
        tag.kvlm[None] = b"A tag generated by lite, which won't let you customize the message!\n"
...
This function is aware of:
 ...
 - the HEAD literal
    ...
    if as_tag: # if tag found
        candidates.append(as_tag)

    as_branch = ref_resolve(repo, "refs/heads/" + name)
    if as_branch: # if branch found
...
def index_read(repo):
    ...
    assert version == 2, "lite only supports index file version 2"
...
def commit_create(repo, tree, parent, author, timestamp, message):
    commit = GitCommit() # new commit object
...
def cmd_commit(args):
    ...
    if not author:
        raise Exception("User name and email not configured. Please set them using gitconfig.")
    ...
    if active_branch:
        with open(repo_file(repo, os.path.join("refs/heads", active_branch)), "w") as fd:
            fd.write(commit + "\n")
        print(f"Committed to branch {active_branch} as {commit}")
    else:
        with open(repo_file(repo, "HEAD"), "w") as fd:
            fd.write(commit + "\n")
        print(f"Committed to detached HEAD as {commit}")
